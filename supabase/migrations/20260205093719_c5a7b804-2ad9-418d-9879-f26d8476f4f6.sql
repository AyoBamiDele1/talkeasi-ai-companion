-- Fix ambiguous column reference in check_milestones RPC
-- The RETURNS TABLE output columns create PL/pgSQL variables; qualify/alias to avoid ambiguity.
...
BEGIN
  ...
  -- Parse safely and round down to whole minutes.
  SELECT 
    COALESCE(SUM(
      CASE 
  ...
  -- Return uncelebrated milestones (explicit aliases to avoid ambiguity)
  RETURN QUERY
  SELECT 
    um.milestone_type AS milestone_type,
    (NOT um.celebrated) AS is_new
